...
public class gameOverScreen extends Application {
    ...
    private String msgToShow = "Oops! You clicked on a bomb!";

    public gameOverScreen(String formattedTimeSpent, boolean timeIsup) {
    ...
    public void start(Stage primaryStage) {
        ...
        Image gameOverIcon = new Image(gameOverScreen.class.getResourceAsStream("game-over.png"));
        ImageView iconImageView = new ImageView(gameOverIcon);
        iconImageView.setFitWidth(160);
        iconImageView.setFitHeight(160);

        // label with the game over message
        Label gameOverLabel = new Label(msgToShow);
        gameOverLabel.setStyle("-fx-font-size: 30px; -fx-text-fill: black; -fx-font-weight: bold;");
        ...
        root.getChildren().addAll(iconImageView, gameOverLabel, timeLabel, buttonBox);

        // Create the scene
        Scene scene = new Scene(root, 800, 600);

        // Load CSS
        scene.getStylesheets().add(gameOverScreen.class.getResource("mycss.css").toExternalForm());
    ...
    private void openDifficulty() throws Exception {
        primaryStage.close(); // Close the game over screen
